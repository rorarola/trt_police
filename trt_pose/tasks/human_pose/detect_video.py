...
def parse_image(img, src, t, X_compress, Y_compress, frame_count):
    color = (0, 255, 0)
    data = preprocess(img)
    ...
    ## 抓第一個人就好了
    keypoints = get_keypoint(objects, 0, peaks, frame_count)
    ...
    # Export to CSV
    with open(csv_folder_root + input_video + '_coord.csv', mode='a', newline='') as f:
        csv_xy_row.insert(0, count)
...
def get_pose_coord(fullpath):
    ...
    with open(csv_folder_root + video_name + '_coord.csv', mode='w', newline='') as f:
        csv_writer = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    ...
    while True:
        ...
        src = parse_image(img, dst, t, X_compress, Y_compress, frame_count)
        out_video.write(src)
        frame_count += 1
